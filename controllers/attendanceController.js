...
const addAttendanceCollection = async (req, res) => {
  try {
    ...
    const duplicateAttendanceEntry = await attendanceModel.find({classId: classId}).exec();
    if (duplicateAttendanceEntry) {
      const filteredAttendance = duplicateAttendanceEntry.filter(entry => entry.date === date);
      if (filteredAttendance.length !== 0) return res.status(409).json({message: 'Attendance data already exists'});
    }

    /////////////////////////
    const attendanceData = req.body;
    if (!attendanceData) return res.status(400).json({message: 'No data sent with body', success: false});
...
const deleteAttendance = async (req, res) => {
  try {
    ...
    if (!attendance) return res.status(400).json({message: 'Error deleting attendance', success: false});
...
const getAllAttendance = async (req, res) => {
  try {
    ...
    if (!attendance) return res.status(404).json({message: 'No attendance data found', success: false});
...
const filterAttendance = async (req, res) => {
  try {
    const {rollNo, schoolId, isPresent, staffName} = req.body;
    const pipeline = [];
    ...
    if (isPresent) {
      pipeline.push({$match: {isPresent: isPresent}});
    }
    if (studentName) {
      pipeline.push({
...
const getClasswiseAttendance = async (req, res) => {
  try {
    const attendance = await attendanceModel.find({classId: req.params.id}).exec();
    if (!attendance) return res.status(404).json({message: 'No attendance data found', success: false});
    res.status(200).json(attendance);
  } catch (error) {
    console.log(error);
...
const getClasswise_DateAttendance = async (req, res) => {
  try {
    ...
    const filteredAttendance = attendance.filter(entry => entry.date === date);
...
const addAttendanceToArray = async (req, res) => {
  try {
    const id = req.params.id;
    if (!id) return res.status(400).json({message: 'No id sent with params', success: false});
    const data = req.body;
    if (!data) return res.status(400).json({message: 'No data sent with body'});
    const attendanceData = await attendanceModel.updateOne({_id: id}, {$push: {attendance: data}});
...
const deleteAStudentAttendance = async (req, res) => {
  try {
    ...
    if (!studentId) return res.status(400).json({message: 'No studentId sent with body', success: false});
    const attendance = await attendanceModel.updateOne({_id: attendanceId}, {$pull: {attendance: {studentId: studentId}}});
    res.status(200).json(attendance);
  } catch (error) {
    console.error(error);
    res.status(500).json({message: 'Server error', success: true});
