...
const filterAttendance = async (req, res) => {
  try {
    const {staffId, schoolId, isPresent, staffName} = req.body;
    const pipeline = [];
    ...
    if (staffName) {
      pipeline.push({
        $match: {isPresent: {$regex: new RegExp(staffName, 'i')}},
      });
    }
...
const deleteAStaffAttendance = async (req, res) => {
  try {
    ...
    if (!staffId) return res.status(400).json({message: 'No studentId sent with body', success: false});
    const attendance = await attendanceModel.updateOne({_id: attendanceId}, {$pull: {attendance: {staffId: staffId}}});
    res.status(200).json(attendance);
  } catch (error) {
    console.error(error);
    res.status(500).json({message: 'Server error', success: true});
